...
def test(request):
    disp_img = Images.objects.all()
    return render(request, 'test.html', {'disp_img': disp_img})

def home(request):
    disp_img = Images.objects.all()
    # Paginator logic 
    paginator = Paginator(disp_img,1)
...
def contact(request):
    if request.method == 'POST':
        ...
        massege = request.POST.get('message')

        # saving the contact form details at database
        cont = Contact(user_name=name, user_email=email, user_subject=subject, user_massege=massege)
        cont.save()
        ...
        send_mail(
                        ...
                        massege,
                        email,
                        ['sachin.pal.itc@gmail.com'],
                        fail_silently=False,
                    )

        # print(name, email,subject,massege)
